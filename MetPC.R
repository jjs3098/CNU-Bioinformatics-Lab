...
estpar_tf <- function(iter,ob,rho=0.5,eta0=0.5,eta1=0.5,alpha=c(1,1,1),beta=c(1,1,1),tau=0.5,piT=0.5,muT,muF,sigmaT,sigmaF){
  y <- 0
  z <- ob$Z
  w <- 0
  yw <- 0
  ...
  for(i in 1:iter){
    ...
    eta0s[i] <- E_eta0(y,z)
    eta1s[i] <- E_eta1(y,z)
    alphas[i,] <- optim(alpha, Qa, method = "BFGS", control = list(fnscale = -1))$par
    betas[i,] <- optim(beta, Qb, method = "BFGS", control = list(fnscale = -1))$par
    taus[i] <- E_tau(y,z,yw)
    piTs[i] <- E_prop(p)
    muTs[i] <- E_muT(p,s)
    muFs[i] <- E_muF(p,s)
...
estpar_ttf <- function(iter,ob,rho=0.5,eta0=0.5,eta1=0.5,alpha=c(1,1,1),beta=c(1,1,1),tau=0.5,pis=c(0.3,0.3),muT1,muT2,muF,sigmaT1,sigmaT2,sigmaF){
  y <- 0
  z <- ob$Z
  w <- 0
  yw <- 0
...
estpar_tff <- function(iter,ob,rho=0.5,eta0=0.5,eta1=0.5,alpha=c(1,1,1),beta=c(1,1,1),tau=0.5,pis=c(0.3,0.3),muT,muF1,muF2,sigmaT,sigmaF1,sigmaF2){
  y <- 0
  z <- ob$Z
  w <- 0
  yw <- 0
...
MetID <- function(iter,l_list,cutoff=0.8,ob,rho=0.5,eta0=0.5,eta1=0.5,alpha=c(1,1,1),beta=c(1,1,1),tau=0.5,piT=0.5,pis=c(0.3,0.3),muT,muT2=NA,muF,muF2=NA,sigmaT,sigmaT2=NA,sigmaF,sigmaF2=NA){
  z <- ob$Z
  s <- ob$S
  bj <- ob$b
  bjs <- ob$b_star
  if(!(is.na(muT2)|is.na(muF2)|is.na(sigmaT2)|is.na(sigmaF2))){
    ...
    parameters <- estpar_tf(iter,ob,rho,eta0,eta1,alpha,beta,tau,piT,muT,muF,sigmaT,sigmaF)
    Py <- estY(rho,eta0,eta1,bj,bjs,alpha,beta,z,tau,s,muT,muF,sigmaT,sigmaF)
  }else if(!(is.na(muF2)|is.na(sigmaF2))){
    parameters <- estpar_tff(iter,ob,rho,eta0,eta1,alpha,beta,tau,pis,muT,muF,muF2,sigmaT,sigmaF,sigmaF2)
    prop <- pis[2]/(1-pis[1])
    Py <- estmfY(rho,eta0,eta1,bj,bjs,alpha,beta,z,tau,s,muT,muF,muF2,sigmaT,sigmaF,sigmaF2,prop)
  }else if(!(is.na(muT2)|is.na(sigmaT2))){
    parameters <- estpar_ttf(iter,ob,rho,eta0,eta1,alpha,beta,tau,pis,muT,muT2,muF,sigmaT,sigmaT2,sigmaF)
    prop <- pis[2]/(1-pis[1])
    Py <- estmtY(rho,eta0,eta1,bj,bjs,alpha,beta,z,tau,s,muT,muT2,muF,sigmaT,sigmaT2,sigmaF,prop)
  }else{
    stop("Invalid variable input")
